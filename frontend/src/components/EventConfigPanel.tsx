...
import { Trash2, Plus, Settings, ChevronRight, ChevronDown, Loader2, Lock, Clock } from "lucide-react";
...
export default function EventConfigPanel() {
        ...
        startExpandedTransition(async () => {
            try {
                ...
                if (session) {
                    ...
                    setEditStart(toLocalInput(session.startTime));
                    setEditEnd(toLocalInput(session.endTime));
    ...
    const handleUpdateSessionDates = async () => {
        ...
        try {
            await votingSessionsApi.update(votingSession.id, {
                ...votingSession,
    ...
    useEffect(() => {
        if (votingSession) loadScalesWithCriteria(votingSession.id);
    }, [votingSession]);
    ...
    const handleAddCriterion = async (scaleId: string) => {
        ...
        try {
            ...
            setCriteriaByScale(prev => ({ ...prev, [scaleId]: [...(prev[scaleId] || []), res.data] }));
    ...
    const handleCloseVoting = (session: VotingSession) => {
        startCloseTransition(async () => {
            try {
                const updated = await votingSessionsApi.update(session.id, { ...session, endTime: new Date().toISOString() });
                setVotingSession(updated.data);
                setEditEnd(toLocalInput(updated.data.endTime));
    ...
    const handleExtend24h = (session: VotingSession) => {
        startExtendTransition(async () => {
            try {
                const newEnd = new Date(session.endTime);
                newEnd.setHours(newEnd.getHours() + 24);
                const updated = await votingSessionsApi.update(session.id, { ...session, endTime: newEnd.toISOString() });
                setVotingSession(updated.data);
                setEditEnd(toLocalInput(updated.data.endTime));
                toast.success("Votación extendida 24h.");
            } catch {
                toast.error("Error al extender la votación.");
            }
        });
    };
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="space-y-3">
                <h3 className="font-semibold text-base">Categorías Existentes</h3>
                {loadingCategories ? (
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex flex-wrap gap-4 items-end bg-background p-4 rounded-lg border shadow-sm">
                                            <div className="space-y-1">
                                                ...
                                                <Input type="datetime-local" value={editStart} onChange={e => setEditStart(e.target.value)} className="h-9 w-44 text-sm" />
                                            </div>
                                            <div className="space-y-1">
                                                <Label className="text-xs text-muted-foreground">Fin de Votación</Label>
                                                <Input type="datetime-local" value={editEnd} onChange={e => setEditEnd(e.target.value)} className="h-9 w-44 text-sm" />
                                            </div>
                                            <Button size="sm" variant="secondary" onClick={handleUpdateSessionDates}>Guardar Fechas</Button>
                                            <div className="flex gap-2 ml-auto">
                                                <Button
                                                    ...
                                                    className="gap-1.5"
                                                    disabled={isClosing}
                                                    onClick={() => handleCloseVoting(votingSession)}
                                                >
                                                ...
                                                <Button
                                                    ...
                                                    onClick={() => handleExtend24h(votingSession)}
                                                >
                                                    {isExtending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Clock className="h-3.5 w-3.5" />}
                                                    +24h
                                                </Button>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex gap-3 items-end">
                                            <div className="flex-1 space-y-1">
                                                ...
                                                <Input placeholder="Ej: Evaluación Técnica Final" value={newScaleDesc} onChange={e => setNewScaleDesc(e.target.value)} className="h-9" />
                                            </div>
